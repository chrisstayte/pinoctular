...
export function TimelineHistogram({ logs, timeRange, onTimeRangeChange }: TimelineHistogramProps) {
  ...
  const maxTotal = buckets.length > 1 ? Math.max(...buckets.map((b) => b.total), 1) : 1
  ...
  const getIndexFromX = (clientX: number): number => {
    ...
    return Math.max(0, Math.min(buckets.length - 1, Math.floor(fraction * buckets.length)))
  }
  ...
  useEffect(() => {
    ...
    const handleGlobalMouseMove = (e: MouseEvent) => {
      ...
      setDragState((prev) => {
        ...
        if (prev.type === "selecting") {
          ...
          onTimeRangeChange([buckets[newStart].startTime, buckets[rangeIndices[1]].endTime])
        } else if (prev.type === "drag-end" && rangeIndices) {
          const newEnd = Math.max(index, rangeIndices[0])
          onTimeRangeChange([buckets[rangeIndices[0]].startTime, buckets[newEnd].endTime])
        } else if (prev.type === "drag-range" && rangeIndices && prev.dragOffsetIndex !== undefined) {
          const rangeLen = rangeIndices[1] - rangeIndices[0]
          let newStart = index - prev.dragOffsetIndex
          newStart = Math.max(0, Math.min(newStart, buckets.length - 1 - rangeLen))
          const newEnd = newStart + rangeLen
          onTimeRangeChange([buckets[newStart].startTime, buckets[newEnd].endTime])
        }
    ...
    const handleGlobalMouseUp = () => {
      if (dragState.type === "selecting" && dragState.startIndex === dragState.currentIndex) {
            ...
            return (
              <div
                ...
                className={`relative transition-opacity duration-75 ${
                  isOutOfRange && !isInDragPreview ? "opacity-20" : "opacity-100"
                }`}
        ...
        {rangeIndices && (
          <>
            ...
            <div
              ...
              style={{ left: `${selStartFrac * 100}%`, transform: "translateX(-50%)" }}
            >
            ...
            <div
              className="absolute top-0 bottom-0 w-1 bg-primary rounded-sm cursor-ew-resize z-10"
              style={{ left: `${selEndFrac * 100}%`, transform: "translateX(-50%)" }}
            >
              <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 left-1/2 w-3 h-6 rounded-sm bg-primary border border-primary-foreground/20 shadow-md flex items-center justify-center">
                <div className="flex gap-px">
                  <div className="w-px h-2 bg-primary-foreground/50 rounded-full" />
        ...
        {dragPreviewIndices && !rangeIndices && (
          <div
            ...
            style={{
              ...
              width: `${((dragPreviewIndices[1] - dragPreviewIndices[0] + 1) / buckets.length) * 100}%`,
            }}
          />
        )}
